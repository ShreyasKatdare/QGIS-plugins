...
FORM_CLASS, _ = uic.loadUiType(os.path.join(
    os.path.dirname(__file__), 'mover3_dialog_base.ui'))


class mover3Dialog(QtWidgets.QDialog, FORM_CLASS):
    def __init__(self, iface, parent=None):
        """Constructor."""
        super(mover3Dialog, self).__init__(parent)
    ...
    def load_map(self):
        ...
        if not original_layer.isValid():
            ...
            QgsProject.instance().addMapLayer(layer)
            self.mytool = MyTool(self.canvas,self.layer, self.farmplots_layer)
            self.mytool.display_rating(self.layer, self.param_selected)
...
class MyTool(QgsMapTool):
    def __init__(self, canvas, layer, farmplots):
        ...
        self.history_vertices = []
        self.param_selected = 'farm_rating'
        self.method = 'worst_3_avg'
        self.farmplots_layer = farmplots
        self.drag_start = None
        self.drag_threshhold = 5

    def canvasMoveEvent(self, event):
        if event.buttons() == Qt.LeftButton:
            point = self.toMapCoordinates(event.pos())
            self.clearHighlight()
            self.highlightVertex(point)
            print(event.pos())
            if (QgsPointXY(point.x(), point.y()) - self.drag_start).length() >= self.drag_threshhold:
                self.newvertex = point
                self.after_new_vertex()
                self.drag_start = QgsPointXY(point.x(), point.y())
    ...
    def canvasPressEvent(self, event):
        if self.highlighted_vertex:
            ...
            self.drag_start = QgsPointXY(self.selected_vertex.x(), self.selected_vertex.y())
            self.after_selection()
            
    
    def canvasReleaseEvent(self, event):
        point = event.pos()
        self.newvertex = self.toMapCoordinates(point)
    ...
    def after_selection(self):
        ...
        for feature in features:
            geom = feature.geometry()
            for vertex in geom.vertices():
                if QgsPointXY(vertex.x(), vertex.y()) == self.selected_vertex:
    ...
    def after_new_vertex(self):
        ...
        # self.new_vertices.append(self.newvertex)
        # self.history_vertices.append(self.selected_vertex)
        for feature in self.features_of_concern:
            geom = feature.geometry()
            new_vertices = []
            for vertex in geom.vertices():
                if QgsPointXY(vertex.x(), vertex.y()) == self.selected_vertex:
                    new_vertices.append(QgsPointXY(self.newvertex.x(), self.newvertex.y()))
                else:
